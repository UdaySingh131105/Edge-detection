...
import com.example.flamedgeviewer.gl.EdgeGLSurfaceView;
...
public class MainActivity extends AppCompatActivity {
    static { System.loadLibrary("flamedgeviewer"); }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        try {
            Log.i("FlamEdgeViewer", "OpenCV test rows=" + testOpenCV());
            Log.i("FlamEdgeViewer", nativeHello());
        } catch (Throwable t) {
            Log.e("FlamEdgeViewer", "JNI test failed", t);
        }
    ...
    private void startCamera() {
        ...
        cameraProviderFuture.addListener(() -> {
            try {
                ...
                Log.e("FlamEdgeViewer", "startCamera failed", e);
            }
    ...
    private void analyzeFrame(ImageProxy proxy) {
        ...
        android.util.Log.d("FlamEdgeViewer", "analyzeFrame format=" + format + " planes=" + planeCount + " expectedNV21Bytes=" + nv21.length);
        ...
        if (planeCount == 1) {
            ...
            android.util.Log.d("FlamEdgeViewer", "single-plane copy size=" + toCopy);
        } else {
            ...
            android.util.Log.d("FlamEdgeViewer", "Y stride=" + yRowStride + " U stride=" + uRowStride + " V stride=" + vRowStride + " U pixStride=" + uPixelStride + " V pixStride=" + vPixelStride
        ...
        if (nv21.length != width * height + (width * height) / 2) {
            android.util.Log.w("FlamEdgeViewer", "nv21 length mismatch: " + nv21.length + " expected=" + (width * height + (width * height) / 2));
        }
