...
Java_com_example_flamedgeviewer_MainActivity_testOpenCV(JNIEnv* env, jobject) {
...
Java_com_example_flamedgeviewer_MainActivity_nativeHello(JNIEnv* env, jobject) {
    return env->NewStringUTF("FlamEdgeViewer JNI ready");
}

extern "C"
JNIEXPORT void JNICALL
Java_com_example_flamedgeviewer_MainActivity_setViewerMode(JNIEnv*, jobject, jint mode) {
    gMode = (mode == 1) ? 1 : 0;
}

extern "C"
JNIEXPORT void JNICALL
Java_com_example_flamedgeviewer_MainActivity_processFrame(
        JNIEnv* env,
        jobject,
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "FlamEdgeViewer", "processFrame called w=%d h=%d mode=%d", width, height, gMode);
    ...
    if (inLen < expected) {
        __android_log_print(ANDROID_LOG_WARN, "FlamEdgeViewer", "inputArray length %d < expected NV21 size %d", inLen, expected);
    }
    ...
    if (nonZeroPct < 2.0f) {
        __android_log_print(ANDROID_LOG_WARN, "FlamEdgeViewer", "processed image almost empty (%.2f%%); falling back to copying Y plane", nonZeroPct);
    ...
    if ((frameCounter % 10) == 0) {
        ...
        __android_log_print(ANDROID_LOG_INFO, "FlamEdgeViewer", "processFrame stats: min=%d max=%d nonZero=%.1f%%", minV, maxV, ratio);
    }
